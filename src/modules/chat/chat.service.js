...
/**
 * Instagram-style inbox: Primary if the recipient follows the sender, else Requests.
 * @param {import('mongoose').Types.ObjectId|string} recipientId
 * @param {import('mongoose').Types.ObjectId|string} senderId
 */
async function dmInboxFolderForRecipient(recipientId, senderId) {
  const follows = await Follow.exists({
    followerId: recipientId,
    followingId: senderId,
  });
  return follows ? 'focused' : 'other';
}
...
function serializeConversation(conversation, membership, peer, extras = {}) {
  ...
  return {
    ...
    youBlocked: Boolean(extras.youBlocked),
    blockedByPeer: Boolean(extras.blockedByPeer),
...
class ChatService {
  ...
  async assertNotBlocked(userId, peerId) {
    ...
    const blocked = await this.findBlock(a, b);
    if (!blocked) return;

    const youBlocked = String(blocked.blockerId) === a;
    throw new AppError(
      youBlocked
        ? 'You blocked this person. Unblock to send a message.'
        : 'This person is not accepting messages from you.',
      HTTP_STATUS.FORBIDDEN,
      { code: 'USER_BLOCKED', details: { youBlocked } },
    );
  }
   ...
   * @param {string} peerId
   */
  async findBlock(userId, peerId) {
    const a = String(userId);
    const b = String(peerId);
    if (!a || !b || a === b) return null;
    if (
      !mongoose.Types.ObjectId.isValid(a) ||
      !mongoose.Types.ObjectId.isValid(b)
    ) {
      return null;
    }
  ...
  /**
   * @param {string|import('mongoose').Types.ObjectId} viewerId
   * @param {string[]} peerIds
   */
  async loadBlockSets(viewerId, peerIds) {
    const selfId = String(viewerId);
    const unique = [...new Set((peerIds || []).map(String).filter(Boolean))];
    const youBlockedSet = new Set();
    const theyBlockedSet = new Set();
    if (!unique.length) {
      return { youBlockedSet, theyBlockedSet };
    }
    const oids = unique.map((id) => new mongoose.Types.ObjectId(id));
    const rows = await ChatBlock.find({
      $or: [
        { blockerId: viewerId, blockedId: { $in: oids } },
        { blockedId: viewerId, blockerId: { $in: oids } },
      ],
    })
      .select('blockerId blockedId')
      .lean();
    for (const row of rows) {
      const blocker = String(row.blockerId);
      const blocked = String(row.blockedId);
      if (blocker === selfId) youBlockedSet.add(blocked);
      if (blocked === selfId) theyBlockedSet.add(blocker);
    }
    return { youBlockedSet, theyBlockedSet };
  }
  ...
  async toConversationDto(conversation, membership, viewerId) {
    ...
    if (!isGroup) {
      const peerId = this.peerIdFromConversation(conversation, viewerId);
      if (peerId) {
        const block = await this.findBlock(viewerId, peerId);
        if (block) {
          youBlocked = String(block.blockerId) === String(viewerId);
          blockedByPeer = !youBlocked;
        }
      }
    }
  ...
  async listConversations(user, { filter = 'focused', q = '', limit } = {}) {
    ...
    if (filter === 'other' || filter === 'requests') memQuery.folder = 'other';
    if (filter === 'focused' || filter === 'primary') memQuery.folder = 'focused';
    ...
    const { youBlockedSet, theyBlockedSet } = await this.loadBlockSets(
      user._id,
      peerIds,
    );
    ...
    for (const conversation of rows) {
      ...
      const isGroup =
        conversation.type === 'group' || conversation.kind === 'group';
      // * Empty request threads stay hidden until the first message arrives.
      if (
        membership.folder === 'other' &&
      ...
      items.push(
        serializeConversation(conversation, membership, peer, {
          memberCount: conversation.participantIds?.length || 0,
          readReceiptsEnabled: viewerAllows,
          youBlocked: Boolean(peerId && youBlockedSet.has(peerId)),
          blockedByPeer: Boolean(peerId && theyBlockedSet.has(peerId)),
        }),
      );
    }
  ...
  /**
   * People picker for New message — follow graph, recents, and directory search.
   * Does not dump the full user table; search is how you reach anyone else.
   */
  async listPeople(user, { q = '', limit } = {}) {
    const take = Math.min(Math.max(Number(limit) || 48, 1), 80);
    const query = String(q || '')
      .trim()
      .replace(/^@/, '');
    const selfId = String(user._id);

    const [followingRows, followerRows, blockRows] = await Promise.all([
    ...
    const followingSet = new Set(
      followingRows.map((row) => String(row.followingId)),
    );
    const followerSet = new Set(
      followerRows.map((row) => String(row.followerId)),
    );
    const blockedSet = new Set();
    for (const row of blockRows) {
      const blocker = String(row.blockerId);
      const blocked = String(row.blockedId);
      blockedSet.add(blocker === selfId ? blocked : blocker);
    }
    ...
    const recentById = new Map(
      recentConversations.map((row) => [String(row._id), row]),
    );
    const recentSet = new Set();
    for (const membership of memberships) {
      const conversation = recentById.get(String(membership.conversationId));
      if (!conversation) continue;
      const peerId = this.peerIdFromConversation(conversation, user._id);
      if (peerId && !blockedSet.has(peerId) && peerId !== selfId) {
        recentSet.add(peerId);
      }
    }
    ...
    const toPerson = (doc) => {
      ...
      const followsYou = followerSet.has(id);
      let relation = 'suggested';
      if (following && followsYou) relation = 'friend';
      else if (following) relation = 'following';
      else if (followsYou) relation = 'follower';
      else if (recentSet.has(id)) relation = 'recent';
      return {
        id,
        username: doc.username || '',
        name: String(doc.fullName || doc.username || 'Member').trim(),
        headline: peerHeadline(doc),
        profilePhotoPath:
          doc.profilePhotoPath ||
          doc.officerPhotoPath ||
          doc.instituteLogoPath ||
          null,
        relation,
        followingAuthor: following,
        followsYou,
        isMutual: following && followsYou,
        isRecent: recentSet.has(id),
        sendsAsRequest: !followsYou,
      };
    };
    ...
    if (query.length >= 1) {
      ...
      const docs = await User.find({
        _id: { $ne: user._id, $nin: [...blockedSet] },
      ...
      const items = docs
        .map(toPerson)
        .sort((left, right) => {
          const leftExact = left.username.toLowerCase() === lowered ? 0 : 1;
          const rightExact = right.username.toLowerCase() === lowered ? 0 : 1;
          if (leftExact !== rightExact) return leftExact - rightExact;
          const leftStart = left.name.toLowerCase().startsWith(lowered) ? 0 : 1;
          const rightStart = right.name.toLowerCase().startsWith(lowered)
            ? 0
            : 1;
    ...
    const seen = new Set();
    const pushId = (id) => {
      if (!id || id === selfId || blockedSet.has(id) || seen.has(id)) return;
      seen.add(id);
      orderedIds.push(id);
    };
    recentSet.forEach(pushId);
    [...followingSet].filter((id) => followerSet.has(id)).forEach(pushId);
    followingSet.forEach(pushId);
    followerSet.forEach(pushId);
    ...
    const suggested = await User.find({
      ...
      .limit(12)
      .lean();
    suggested.forEach((doc) => pushId(String(doc._id)));
  ...
  async getOrCreateConversation(user, { peerUserId, peerUsername } = {}) {
    ...
    const peerFolder = await dmInboxFolderForRecipient(peer._id, user._id);
    const peerMembership = await ChatMembership.findOne({
      conversationId: conversation._id,
      userId: peer._id,
    });
    if (!peerMembership) {
    ...
    } else if (peerMembership.deletedForUserAt) {
      peerMembership.deletedForUserAt = null;
      peerMembership.folder = peerFolder;
      peerMembership.archived = false;
      await peerMembership.save();
    }
  ...
  async sendMessage(
    ...
    // * Replying to a request accepts it (Instagram).
    if (membership.folder === 'other') {
      membership.folder = 'focused';
    }
    ...
    for (const oid of otherIds) {
      ...
      const peerFolder = isGroup
        ? 'focused'
        : await dmInboxFolderForRecipient(oid, user._id);
      await ChatMembership.findOneAndUpdate(
        {
          conversationId: conversation._id,
  ...
  async reportAndBlock(
    ...
    await this.upsertBlock(user, peerId);
  ...
  /**
   * Block the peer and keep the thread visible with a Blocked chip.
   */
  async blockConversation(user, conversationId) {
    const { membership, conversation } = await this.requireMembership(
      user,
      conversationId,
    );
  ...
  /**
   * Remove a block the current user placed on the peer.
   */
  async unblockConversation(user, conversationId) {
    const { membership, conversation } = await this.requireMembership(
      user,
      conversationId,
    );
