...
class DayBriefService {
  ...
  /**
   * Active briefs for the Home strip. Guests see the public tray;
   * signed-in viewers get viewed-state. Not limited to follows so
   * community Day Briefs actually appear on Feed.
  ...
  async listFeed({ viewerId }) {
    ...
    const viewerOk =
      Boolean(viewerId) && mongoose.Types.ObjectId.isValid(viewerId);
    let viewedSet = new Set();
    if (viewerOk) {
      const views = await DayBriefView.find({
        briefId: { $in: rows.map((row) => row._id) },
        viewerId: new mongoose.Types.ObjectId(viewerId),
      })
        .select('briefId')
        .lean();
      viewedSet = new Set(views.map((row) => String(row.briefId)));
    }
